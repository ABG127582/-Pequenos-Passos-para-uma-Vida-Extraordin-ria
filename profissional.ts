...
const enterEditMode = (li: HTMLLIElement) => {
    ...
    const saveEdit = () => {
        ...
        if (newText && goalId) {
            ...
            if (goal) {
                goal.text = newText;
                storageService.set(STORAGE_KEYS.PROFISSIONAL_GOALS, goals);
            }
        }
        renderGoals();
    };
...
const handleGoalAction = (e: Event) => {
    ...
    if (target.closest('.edit-btn')) {
        ...
        if (goal.completed && !wasCompleted) {
            ...
            awardMedalForCategory('profissional');
        }
...
export function show() {
    const savedGoals: Goal[] | null = storageService.get(STORAGE_KEYS.PROFISSIONAL_GOALS);
