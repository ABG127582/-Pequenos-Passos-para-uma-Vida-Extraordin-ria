...
interface Task {
    ...
    description: string;
    dueDate: string;
    time?: string; // Optional time for alarm
...
const openTaskModal = (task?: Task) => {
    ...
    if (task) {
        ...
        elements.modalTimeInput!.value = task.time || '';
...
const renderTableView = (tasksToRender: Task[]) => {
    ...
    tasksToRender.forEach(task => {
        ...
        const timeText = task.time ? `<span class="item-time"><i class="fas fa-clock"></i> ${task.time}</span>` : '';
        row.innerHTML = `
    ...
    orderedCategories.forEach(catName => {
        if (groupedTasks[catName] && groupedTasks[catName].length > 0) {
             ...
             groupedTasks[catName].forEach(task => {
                ...
                const timeText = task.time ? `<span class="checklist-item-time"><i class="fas fa-clock"></i> ${task.time}</span>` : '';
                itemEl.innerHTML = `
...
const checkDueTasks = () => {
    ...
    tasks.forEach(task => {
        if (task.dueDate === today && task.time === currentTime && !task.completed && !notifiedTaskIds.has(task.id)) {
            window.showToast(`Lembrete de tarefa: "${task.title}"`, 'info');
